...
export interface ReactGrabInstallTaskOptions {
  session: CloneSession;
}

export interface ReactGrabInstallTask {
  title: string;
  body: string;
}

export function createReactGrabInstallTask(options: ReactGrabInstallTaskOptions): ReactGrabInstallTask {
  const { session } = options;

  return {
    title: `Install React Grab for ${session.sessionId}`,
    body: `# React Grab Install Task
...
\`\`\`bash
corepack pnpm cli react-grab-task ${session.sessionId} --context <grab-context.json>
\`\`\`
