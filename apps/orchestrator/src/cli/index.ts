...
program
  ...
  .action(async (url: string, options: { refresh?: boolean }) => {
    const projectRoot = getProjectRoot();
    const sessionId = createSessionId(url);
    const existed = await cloneSessionExists(projectRoot, sessionId);
    const session = await initCloneSession({ url, projectRoot, refresh: options.refresh });
    const action = options.refresh && existed ? "Refreshed" : existed ? "Reused" : "Created";
